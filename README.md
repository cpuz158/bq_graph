...
### 1. 🕸️ 4대 차세대 그래프 시각화 엔진 (Multi-Engine Visualization)
- **1. PyVis (2D Physics 물리 엔진)**: `forceAtlas2Based` 물리 시뮬레이션으로 노드가 유기적으로 넓게 퍼지며 자유로운 드래그, 줌, 팬 및 물리 반동 인터랙션 제공.
- **2. Cytoscape.js (구조적 엔지니어링 뷰)**: 생물정보학/네트워크 공학 표준 COSE 계층 레이아웃 및 베지어 곡선(Bezier Curve) 엣지 제공.
- **3. 3D Force Graph (WebGL 3D 우주 궤도 뷰)**: Three.js 기반 3차원 우주 궤도 뷰, 360도 3D 회전/줌, 활성 탐색 경로를 따라 실시간으로 흐르는 에너지 입자(Particle) 효과, 노드 클릭 시 부드러운 카메라 비행 이동.
- **4. AntV G6 (Alibaba 엔터프라이즈 방사형 뷰)**: 알리바바 엔터프라이즈 방사형(Radial) 위상 레이아웃, 시작 노드 중심 1-Hop/2-Hop 노드 포커싱 인터랙션, 네온/골드 글로우(Glow) 효과.
- **마우스 호버 카드 툴팁**: 모든 엔진에서 노드 및 엣지에 마우스를 올리면 테마 배색과 일치하는 상세 메타데이터 카드 툴팁 표시.
